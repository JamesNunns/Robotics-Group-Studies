...
public class MotionLeft : MonoBehaviour
/// <summary>
...
///<\summary>
{
	...
	void Update()
	{
		...
		{
			Move(leftKnee, 40, 1f);
			Move(leftAnkle, -45, 1f);
			Move(leftHip, -70, 1f);
			//Move(leftShoulder, -40, 1f);
			Move(leftElbow, -70, 1f);
		}
		if (Input.GetKey("n"))
		{
			Move(leftKnee, 0, 1f);
		...
		if (Input.GetKey("c")) //c for crouch
		{
			changePosition(low_cm, 0.5f);
		}
		if (Input.GetKey("s")) //s for stand
		{
