...
from torso_and_legs import torso_dict, legs_dict
    ...
    for i in range(len(summary)/4):
        for keys in values:
            if values[keys][0] == summary[4*i]:
                ...
                current_command[keys] = float(summary[4*i + 2])
    return current_posture, current_command

#motionProxy.getSummary()
posture_now = neutral
posture_dict, command_dict = GetPosture(posture_now)

#with open("../Interface/positions_new.py", "r") as read_file:
#    lines = read_file.readlines()
#with open("../Interface/positions_new.py", "w") as vfile:
