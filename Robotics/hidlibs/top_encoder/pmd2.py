...
# Copyright 2009-2018 Mark Colclough
...
# Taking care to specify pointers to avoid clipping to 32 bit on round-trip through python
# Default return type is a C int except where specified
pmd_find_first = libpmd.pmd_find_first
pmd_find_first.restype = c_void_p

pmd_flash = libpmd.pmd_flash
pmd_flash.argtypes = [c_void_p]

pmd_serial = libpmd.pmd_serial
pmd_serial.restype = c_char_p
pmd_serial.argtypes = [c_void_p]

pmd_digin = libpmd.pmd_digin;
pmd_digin.argtypes = [c_void_p, c_int]

pmd_digin16 = libpmd.pmd_digin16
pmd_serial.argtypes = [c_void_p]

pmd_digout = libpmd.pmd_digout
pmd_digout.argtypes = [c_void_p, c_int, c_int]

pmd_digconf = libpmd.pmd_digconf
pmd_digconf.argtypes = [c_void_p, c_int, c_int]

pmd_aout = libpmd.pmd_aout
pmd_aout.argtypes = [c_void_p, c_int, c_int]

pmd_ain = libpmd.pmd_ain
pmd_ain.argtypes = [c_void_p, c_int, c_int]

pmd_ainstop = libpmd.pmd_ainstop
pmd_ainstop.argtypes = [c_void_p]

pmd_ainscan = libpmd.pmd_ainscan
pmd_ainscan.argtypes = [c_void_p, c_int, c_int, c_int, c_int, c_int, c_void_p, c_int, c_int]

pmd_ainawaited = libpmd.pmd_ainawaited
pmd_ainawaited.argtypes = [c_void_p]

pmd_ainactive = libpmd.pmd_ainactive
pmd_ainactive.argtypes = [c_void_p]

pmd_ainkill = libpmd.pmd_ainkill
pmd_ainkill.argtypes = [c_void_p]

pmd_crst = libpmd.pmd_crst
pmd_crst.argtypes = [c_void_p]

pmd_cin = libpmd.pmd_cin
#pmd_cin.restype = c_ulong # actually, only 32 bits are valid
pmd_cin.argtypes = [c_void_p]
...
usb_get_errmsg.argtypes = [c_int]

pmd_close = libpmd.pmd_close
pmd_close.argtypes = [c_void_p]


class pmd1208fs(object):
    ...
    def close(self):
        ...
        """
        #print("closing pnd handle %s" % self.pmd)
    ...
    def version(self):
        ...
        -------
        string
            Version of libpmd1208fs in use. Py2: unicode, Py3: str.
        """
        return pmd_version().decode()
    ...
    def hserial(self):
        ...
        -------
        string
            Hardware serial number (8 characters) Py2: unicode, Py3: str.
        """
        return pmd_serial(self.pmd).decode()
    ...
    def aout(self, channel, value):
        ...
        if ret < 0:
            raise IOError("%d: %s" % (ret, usb_get_errmsg(ret).decode()))
    ...
    def ain(self, channel, adcrange):
        ...
        if ret < -2048:
            usberr = ret+2049
            raise IOError("%d: %s" % (usberr, usb_get_errmsg(usberr).decode()))
        return ret
...
if __name__ == "__main__":
    ...
    for i in range(5):
        try:
            ...
            input("failed to find! CR to try again")
    ...
    print("doing a less boring ainscan, as soon as it will work (may take 60 s)")
    while 1:
        try:
            mydev.ainscan(block=0)
            break
