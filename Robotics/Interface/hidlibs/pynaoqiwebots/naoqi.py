                ...
                if (bindIfnoDocumented or function.__doc__ != ""):
                    if (thing[0] != "_"):  # private method
                        ...
                        for param in function.func_code.co_varnames:
                            ...
                            myClass._bindWithParam(
                                myClass.getName(), thing, _getMethodParamCount(function) - 1)
...
# define a class that will be inherited by both ALModule and ALBehavior,
# to store instances of modules, so a bound method can be called on them.
...
class NaoQiModule():
    ...
    def __init__(self, name, logger=True):
        # keep a weak reference to ourself, so a proxy can be called on this
        # module easily
    ...
    def __del__(self):
        # when object is deleted, clean up dictionnary so we do not keep a weak
        # reference to it
...
class ALBehavior(inaoqi.behavior, NaoQiModule):
    ...
    def __init__(self, param, autoBind, brokerRegister=True):
        ...
        if(autoBind):
            ...
            for methName in userMethList:
                ...
                if callable(function) and isinstance(
                        function, type(self.__init__)):
                    if (methName[0] != "_"):  # private method
                        ...
                        for param in function.func_code.co_varnames:
                            if (param != "self"):
                                self.addParam(param)
                        self._bindWithParam(
                            behName, methName, _getMethodParamCount(function) - 1)
        if brokerRegister:
            self.registerToBroker()
    ...
    # Depreciate this!!! Same as self.logger.info(), but function is always
    # "log"
    def log(self, p):
        self.logger.info(p)
        ...
        try:
            ...
            class MethodMissing(object):
                ...
                def __call__(self, *args, **kwargs):
                    return self.__wrapped__.method_missing(
                        self.__method__, *args, **kwargs)
