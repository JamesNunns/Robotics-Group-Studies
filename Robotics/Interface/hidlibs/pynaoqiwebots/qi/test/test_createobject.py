...
def main():
    ...
    # but this does not include the _d suffix on windows (tested on cmake
    # 2.8.10.2)
    if sys.platform.startswith("linux"):
        full_path = "%s/lib/libtestregisterobject%s.so" % (sys.argv[2], pyver)

    if sys.platform.startswith("darwin"):
        full_path = "%s/lib/libtestregisterobject%s.dylib" % (
            sys.argv[2], pyver)

    if sys.platform.startswith("win"):
        full_path = "%s/bin/testregisterobject%s.dll" % (sys.argv[2], pyver)
        if not os.path.isfile(full_path):
            full_path = "%s/bin/testregisterobject%s_d.dll" % (
                sys.argv[2], pyver)
        if not os.path.isfile(full_path):
