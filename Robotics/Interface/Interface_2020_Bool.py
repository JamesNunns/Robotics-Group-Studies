...
from utility_functions import flatten, read_file, current_data_types, get_latest_file, convert_list_dict, store
...
class Interface(Algorithm):
    ...
    def __run_real(self, t, period):
        ...
        for event in range(int(max_runs)):
            ...
            l_move, t_move = self.is_moving('legs'), self.is_moving('torso')
            current_values = convert_list_dict([time, event, ax, ay, az, gx, gy, gz, se0, se1, se2, se3, be, av, cmx, cmy, algo, position, l_move, t_move])
