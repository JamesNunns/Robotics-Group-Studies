...
class TP_Predict():
    ...
    def algo(self,values,all_data):
        ...
        if values['time'] - self.start_time < self.duration:
            ...
            next_max, next_min, next_zero = next_points_hp(values, l)
            ...
            if values['time'] - l[1] < self.offset:
                if l[1] - values['time'] < 0:
                    return "low_cm"
            if next_zero - values['time'] < self.offset:
                if next_zero - values["time"] > 0:
                    return "high_cm"
            if values['time'] - l[2] < self.offset:
                if l[2] - values["time"] < 0:
                    return "high_cm"
