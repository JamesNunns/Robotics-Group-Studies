...
class NodeGene():
    ...
    # returns list containing all connected nodes
    def all_connections(self):
        
        return self.back_conn + self.forward_conn
    ...
    def get_connections(self, _print=True):
        ...
        if _print:
            ...
            print('Node {}: (type={})'.format(self.num, self.type))                   
...
class Chromosome():
    ...
    # prints node information for the chromosome
    def get_nodes(self):
        
        for node in self.node_genes: node.get_connections()
    ...
    def compile_network(self):
        ...
        for node_gene in node_genes:
            ...
            if node_gene.layer != 0: biases[node_gene.eff_layer-1][node_gene.eff_node_index] = node_gene.bias
    ...
    # function to mutate the chromosome in accordance with NEAT method
    def mutate(self,
               gin,
               add_node_prob = 0.1,
        ...
        if random.random() < add_node_prob:
            ...
            new_node = NodeGene(num = len(self.node_genes), 
            ...
            # append connections to chromosome
        ...
        if random.random() < add_connection_prob:
            ...
            try: 
                ...
                node1 = random.choice([n for n in self.node_genes 
                                       if len(n.all_connections()) < len([m for m in self.node_genes if m.layer != n.layer]])])
                
                # choose second random node that is unconnected to the first and in a different layer
                node2 = random.choice([n for n in self.node_genes 
                                       if n.layer != node1.layer and n not in node1.all_connections()])
...
con6 = hidden5.add_connection(output7, weight=0.5)
